...
class ProductController extends GetxController {
  ...
  // Keep isPriceOff method for potential future use
  bool isPriceOff(Product product) => product.off != 0;
  ...
  void filterProductsBySearch(String query) {
    if (query.isEmpty) {
      filteredProducts.assignAll(allProducts); // Show all products if search is empty
    } else {
  ...
  void calculateTotalPrice() {
    ...
    for (var element in cartProducts) {
      totalPrice.value += element.quantity * element.price;  // Apply original price here
    }
  }
